class HomesController < ApplicationController
  ...
  def dashboard
    ...
    @fixed_plans = SuppliersPlan.where(plan_type: "fixed").first(3)
    @discounted_plans = SuppliersPlan.where(plan_type: "discount_off").first(3)
    @peakoffpeak_plans = PeakAndOffPeakPlan.first(3)
  end
